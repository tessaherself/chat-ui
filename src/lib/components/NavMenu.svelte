...
<script lang="ts">
	...
	const isIframeMode = $derived(publicConfig.PUBLIC_SHOW_NAV_LOGO !== "true");
...
<div
	...
	class:py-3.5={!isIframeMode}
	class:py-2={isIframeMode}
	style={isIframeMode ? "margin-bottom: -50px;" : ""}
>
	{#if !isIframeMode}
		<a
	...
	<a
		...
		style={isIframeMode
			? "transform: translateY(-2px); margin-right: 10px; box-shadow: 0 6px 24px 0 rgba(40,90,105,0.26), 0 5px 8px 0 rgba(255,255,255,0.48) inset, 0 0 12px 0 rgba(255,255,255,0.20); border: 1.5px solid rgba(122, 159, 215, 0.4); background: transparent; backdrop-filter: blur(1px) saturate(80%); -webkit-backdrop-filter: blur(1px) saturate(80%); color: #555; padding: 4px 8px; font-size: 0.8rem; font-weight: 500; transition: box-shadow 0.18s, background 0.18s;"
			: ""}
		...
		>
			{#if user?.avatarUrl}
				<img
					...
					alt=""
				/>
			{:else if publicConfig.isHuggingChat && user?.username}
				<img
				...
				/>
			{:else}
				<div class="size-3.5 rounded-full border bg-gray-500 dark:border-white/40"></div>
			{/if}
			<span
