...
/**
 * Handles auth failure recovery, especially for iframe-embedded deployments.
 * First attempt: soft recovery via page reload.
 * Repeated failures within 60s: redirect to login page.
 */
export async function triggerAuthRecovery(opts?: {
	...
	// If embedded in an iframe, notify the host app
	try {
	...
	window.sessionStorage.setItem(AUTH_RECOVERY_KEY, JSON.stringify({ count: nextCount, ts: now }));
